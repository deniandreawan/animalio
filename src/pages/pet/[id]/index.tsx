...
const PetPage: FC = () => {
  ...
  const { data: petData, error } = useSWR<{ data: IPet }>(
    id ? `/api/pets/${id}` : null,
    fetcher
  );
      ...
      <div className="flex flex-col laptop:flex-row items-start">
        ...
        <div className="laptop:flex-basis-40 w-full h-80 laptop:h-120 overflow-hidden laptop:sticky laptop:top-20 flex flex-col">
          <div
            ...
            style={{
              background: `#f1f1f1 url(${activeImage?.url || pet.image?.url})`,
            }}
          />
          <div className="mt-4">
            {pet.images?.length > 0 && (
              <div className="grid grid-cols-4 gap-2 h-20 laptop:h-24">
                {[pet.image, ...pet.images].map((image) => (
                  <div
                    className={`w-full opacity-80 h-full rounded-lg shadow-lg hover:cursor-pointer !bg-cover !bg-no-repeat !bg-center ${
                      activeImage?.public_id === image.public_id &&
                      "opacity-100 border-2 border-solid border-accent-400"
                    }`}
                    key={image.public_id}
        ...
        <div className="h-full flex-basis-60 py-4 laptop:p-12  laptop:ml-8 space-y-4">
          <div>
            <h1>
              I'm <span className="text-primary-500">{pet.name}</span>
            </h1>
          </div>
          {pet.description && (
            <div>
          ...
          <div>
            ...
            <h5 className="pet-name">{pet.species}</h5>
          </div>
          <div>
              ...
              <div
                ...
                style={{
                  background: `#f1f1f1 url(${
                    pet.owner.photo?.url || "https://i.pravatar.cc/150"
                  })`,
                }}
                title={pet.country?.label || ""}
              />
          ...
          {pet.isOwnPet && (
            <div className="flex space-x-4 mt-8">
              <Link href="/pet/[id]/edit" as={`/pet/${pet._id}/edit`}>
                <button className="flex items-center" disabled={isDeleting}>
                  <FiEdit3 />
                  &nbsp; Edit
                </button>
              </Link>
