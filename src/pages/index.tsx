import { Filters, Loader } from "@/components/common";
...
const Index: FC = () => {
  ...
  if (selected.country?.value)
    searchParam.append("country", selected.country.value);
  ...
  const renderErrorPage = () => {
    return error?.statusCode === 404 ? (
      <NoPetFound appliedFilters={selected} />
    ) : (
      <SomethingWentWrong />
    );
  };
    ...
    <div className="min-h-screen laptop:flex laptop:items-start my-20 mx-0 py-5 px-5 laptop:px-12">
      ...
      <div className="w-full h-full mt-8 laptop:mt-0">
        {selected.text && !error && pets && (
          <h2 className="mb-5">
            Search result for:{" "}
            <span className="text-primary">{selected.text}</span>
          </h2>
        )}
        {error ? (
          renderErrorPage()
