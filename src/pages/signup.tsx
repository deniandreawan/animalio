...
const SignUp = () => {
  ...
  const onSubmit = async (field: IFormState) => {
    ...
    const req = await fetch("/api/auth/signup", {
      method: "POST",
      body: JSON.stringify({
        //explicitly set fields to avoid unnecessary data to be sent to server
        email: field.email,
      ...
      <div className="w-full h-full relative p-5 flex z-10">
        <div className="hidden laptop:block w-full p-12">
          <Link href="/">
            <img
              alt="Logo"
              className="w-20 object-contain mb-8"
              src="/favicon.png"
              width={100}
              height={80}
            />
          ...
          <h1 className="text-white">
            Make your <span className="line-through font-normal">pet</span>{" "}
            <span className="font-bold">best friend</span>
            <br />
        ...
        <div className="w-full h-full">
          <div className="w-full laptop:w-3/4 h-full bg-white rounded-lg py-8 px-5 laptop:px-12 relative overflow-hidden">
            ...
            {error && (
              <h4 className="text-red-500 bg-red-100 text-center p-2">
                {error}
              </h4>
            )}
            <Formik
