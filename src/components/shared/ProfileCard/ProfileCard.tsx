...
const ProfileCard: FC<IProps> = ({ user }) => {
  ...
  return (
    <div className="min-w-300px text-center flex items-center flex-col bg-primary-500 pt-12 laptop:rounded-lg text-white p-5 shadow-lg">
      <div
        ...
        style={{
          background: `#f1f1f1 url(${
            photo.url || user.photo?.url || "https://i.pravatar.cc/150"
          })`,
        }}
      >
        {isEditMode && (
          <div className="w-full h-full absolute top-0 left-0 rounded-full bg-black bg-opacity-40 flex items-center justify-center overflow-hidden hover:cursor-pointer">
            <label
              className="w-full h-full text-white flex flex-col text-center justify-center items-center hover:opacity-80 hover:cursor-pointer"
              htmlFor="photo"
            >
      ...
      <div>
        ...
        {isEditMode && (
          <div className="flex space-x-2">
            <button disabled={isLoading} onClick={handleCancelEdit}>
              Cancel
            </button>
            <button
