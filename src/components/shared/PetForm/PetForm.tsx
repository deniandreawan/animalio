...
const PetForm: FC<IProps> = ({ formId, petForm, forNewPet = true, title }) => {
  ...
  const [imageFiles, setImageFiles] = useState<IImageFile[]>(() => {
    ...
    const initFiles = petForm.image
      ? [petForm.image, ...petForm.images]
      : petForm.images;
  ...
  const initFormikValues = {
    ...
    breed: petForm.breed,
    country: petForm.country || { value: "ID", label: "Indonesia" },
    age: petForm.age,
    image: petForm.image,
  ...
  const handleSubmit = async (values: IFormState) => {
    ...
    imageFiles.forEach((img) => {
      if (img.isThumbnail || imageFiles.length === 1) {
        // if the image is thumbnail
        if (img.file) {
          // if file exists - Let's upload it
  ...
  return (
    <div>
      <div>
        {error && (
          <span className="p-2 text-red-500 bg-red-100 block">{error}</span>
        )}
        <h1 className="text-gray-800 text-2xl laptop:text-4xl">{title}</h1>
      </div>
      <p className="text-gray-500 flex items-center">
        <FiInfo /> &nbsp; Labels with{" "}
        <strong style={{ color: "var(--primary)" }}>*</strong> are required.
      </p>
      <br />
      <br />
      <br />
                    ...
                    {({ form, meta }) => (
                      <div>
                        ...
                        <CountryDropDown
                          ...
                          disabled={isSubmitting}
                          onChange={(val) =>
                            form.setValues((vals: IFormState) => ({
                              ...vals,
                              country: val,
                            }))
                          }
