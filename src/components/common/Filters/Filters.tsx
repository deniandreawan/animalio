...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`transition-all duration-500 w-full laptop:w-auto laptop:min-w-300px mb-4 fixed ${
        isVisibleFilter ? "top-14 shadow-lg" : "-top-28"
      } left-0 z-30 px-4 laptop:px-0 bg-white laptop:bg-transparent laptop:mb-0 laptop:sticky laptop:top-20 laptop:mr-12 pt-5`}
    >
      {!isSmallScreen && (
        <div className="mb-8">
          <AddNewButton />
      ...
      <label className="text-sm text-gray-700" htmlFor="species">
        ...
        <SpeciesDropDown
          ...
          defaultValue={
            filter.selected.species ? labelize(filter.selected.species) : null
          }
      ...
      <br />
      {Object.values(filter.selected).some(
        (val) => Boolean(val) && !isSmallScreen
      ) && (
        <span
